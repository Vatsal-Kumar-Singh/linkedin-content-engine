...
def main():
    ...
    print("\n%4s %-6s %-5s %-22s %-20s %-7s %s" %
          ("#", "pain", "tier", "job", "angle", "fit", "format Lift would pick"))
    print("-" * 104)
    for i, (score, c, r) in enumerate(chosen, 1):
        rc = recommend(c, a.channel, prof)
        best = rc["format"] or "(unmeasured)"
        print("%4d %-6s %-5s %-22s %-20s %-7s %s"
              % (i, c["pain"], c["tier"], c["job"], c["angle"],
                 "%.3f" % score if score is not None else "  -", best))
